...
│     ├─ shell.rs              # 界面骨架：顶栏（ConnectPanelRoot 抽屉）/ 侧栏（SidebarRoot）/ 内容区 / 状态栏 / 右键菜单层
...
侧栏    文件列表卡片（唯一卡片；单击加载，右键上传/备份/删除。日志分析视图下整个侧栏隐藏，页面全宽）
...
- 侧栏只保留文件列表卡片（参数 / 流程图视图）；日志分析视图隐藏整个侧栏，日志页全宽。
