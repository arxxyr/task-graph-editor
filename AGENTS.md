...
│     ├─ shell.rs              # 界面骨架：顶栏（ConnectPanelRoot 抽屉）/ 工作台（TaskWorkspace）与日志页 / 状态栏 / 右键菜单层
│     ├─ connect.rs            # 顶栏 SSH 摘要条与抽屉表单、ssh config 主机下拉、连接按钮组
...
# 离线载入本地任务图副本，呈现「已连接、已加载文档」的界面（顶栏操作按钮、参数树），不连接远端
...
- ROS2 数组回填可同时设置 `TASK_GRAPH_REAL_FILE=<任务图副本>` 与
...
```
顶栏    ● 任务图编辑器 │ [SSH ● user@host:port ▾] [参数 / 流程图 / 日志分析] 当前文件   [编辑操作] [选择主题]
        抽屉行（可折叠）：主机[______][☰] 用户名[____] 密码[____]   [连接][断开][刷新列表][上传文件]
                          ▸ 连接设置（展开后：端口 / 私钥 / DOMAIN_ID / 任务目录，换行排列）
工作台  TaskWorkspace（参数 / 流程图视图显示，与日志页平级；切到日志视图时整体隐藏）
        ├─ 侧栏    文件列表卡片（唯一卡片；单击加载，右键上传/备份/删除）
        └─ 内容区  ┌ 参数视图：元数据卡片（map_id / task_id）
                   │           Context 参数卡片 → 六个分组：位姿点位 / 基本参数 / 数组参数 / 轨迹数据 / 嵌套分组 / 其他
                   └ 流程图视图：面包屑 + 本层统计 + 图例 / 画布 / 详情栏
日志页  与工作台平级、独占全宽，自带页内文件列表（「选择日志 / 分析设置 / 分析结果」三张卡片）
状态栏  [复制提示] 状态消息 / 忙碌 / 重连（三选一）             user@host:port
```

窄窗口或放大 UI 后，编辑操作自动移到顶栏第二行；SSH 摘要条与视图入口保持在第一行，
视图入口仍对齐右侧内容区左缘，主题菜单保持右上角。

顶栏与主体共用侧栏宽度：左侧是标题和文件选择区，右侧是文档功能区。
「参数 / 流程图 / 日志分析」入口常驻并对齐右侧内容的左缘，独立成组，与编辑操作按钮通过弹性间距分开。
编辑操作组可继续内部换行，顶栏随内容增高。三种视图互斥（`display`），未加载文档时显示引导。
选中高亮只更新 `ButtonVariant`，不重建按钮；跨帧生成的面板也要同步当前视图。

编辑操作组是三个 ROS2 取数按钮（取底盘 / 取头部 / 取腰部）加「保存到远程」，直接摆在顶栏：取数是示教时
最频繁的操作，曾收进「位姿取数」下拉菜单，每次都要多点一下，已按使用反馈改回按钮。宽窗口（≥1200）下
操作组 `flex_shrink: 0`，空间不够时裁剪的是文件名，而不是把按钮挤到第二行；按钮文案因此控制在三个字，
改长之前先用 `TGE_SCREENSHOT_TASK` 夹具在默认 1280 宽度下截图确认仍是单行。

- SSH 连接模块在顶栏：摘要条 = 状态点 + 连接目标 + 展开指示（▾/▴），点击展开或收起完整表单抽屉
  （主机 / 用户名 / 密码横向流式排列，「连接设置」折叠占整行，连接按钮组同行换行）。
  展开状态由 `ConnectPanelOpen` 记录：未连接默认展开，连接成功自动收起，断开自动展开，方便换主机；
  手动点摘要条随时翻转。`TGE_CONNECT_PANEL=collapsed` 配合 `TGE_SCREENSHOT` 截图验收收起态。
- 侧栏是参数 / 流程图任务工作台（TaskWorkspace）的内部模块，只保留文件列表卡片；日志视图与工作台平级、
  自带页内文件列表。切到日志视图时工作台整体隐藏（display），切回任务视图即恢复，文件列表实体与滚动位置不变。
...
- 独立位姿和位姿数组的每个 `[index]` 都用可选卡片；点击标题或内部控件选中后，才能用顶栏的三个 ROS2 取数按钮。
  选择目标用 `PoseTarget` 同时记录字段路径和可选数组下标，嵌套分组内同样适用；选中只刷新配色，不重建控件。
  ROS2 请求保存完整目标和结构版本，返回后回填原目标；切换选中不会转移写入位置，重载文档或结构变化后拒绝旧响应。
  状态提示包含数组下标（如 `pick_poses[0]`），回填仍走原有数值刷新与无损合并序列化。
- 位姿的位置 xyz、姿态 wxyz 各横排一行，色条区分轴向（X 红 / Y 绿 / Z 蓝）
- UI 缩放：Shift + `+`/`-` 或 Shift + 滚轮（`UiScale`，0.5～3.0）
- 保存快捷键：Ctrl/Cmd + S 保存到远程，在所有视图（参数 / 流程图 / 日志分析）生效，原先仅流程图视图；
  未保存确认弹层激活时仍优先拦截，文本框聚焦时同样触发（检查在文本焦点门之前）。
...
- **主题切换**：默认石墨青，右上角「选择主题」提供石墨青、暮砂金、暖纸橙、雾白靛。
  ...
  `theme/refresh.rs` 补齐上游未随资源换色刷新的继承文字、文本光标、滑块和图标，修复输入框默认白字；
  找不到继承来源的裸 `ThemedText` 补挂直接主题色，浅色主题下不退化成默认白字。
...
- 日志页面按「选择日志 / 分析设置 / 分析结果」三张卡片组织，前两张收在「选择与设置」折叠区内；
  文件行图标使用更纱黑体 Nerd 字形常量，定义在 `src/ui/log_analysis.rs`。
