...
卡片边框、分组边框和栏间分隔采用 `border` 22% + `panel` 78% 的 sRGB 混合色，让容器退后、控件边界更清楚。输入框与流程图连线仍使用原始 `border`；下文对比度表中的边框数值也指原始色。

注：混合比例原为 42% + 58%，本次界面现代化下调至 22% + 78%（`theme.rs` 的 `soft_border` 同步调整），让边框进一步退后；对比度表内数值均按原始 `border` 计算，不受混合比例调整影响，未做改动。
