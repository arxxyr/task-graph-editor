...
- **地图点位同步** — 保存任务图后，自动把底盘位姿同步到同一工作区地图 GeoJSON 的关联导航点，只改动变化的数字
...
4. 编辑数据，或点击位姿卡片选中后用顶栏的「取底盘 / 取头部 / 取腰部」按钮从 ROS2 获取实时数据
   （分别对应底盘位姿、头部关节角、腰部关节角）
...
机器人运行时以地图 GeoJSON 的导航点为准，并按 `pose_context_key` 注入任务图 context。因此每次
保存任务图后，编辑器会接着同步 `<工作区>/map/<map_id>/geo_info/*.geojson`（工作区即任务目录的上一级）：

- 只处理 `properties.source_task_id` 等于当前 `task_id`、且地图标识一致的文件；
- 导航点按 `pose_context_key` 关联顶层位姿字段，旧文件没有该键时退回 `source_context_key`；
  同一个键可对应多个导航点，全部更新；
- 写入 `geometry.coordinates` 的 x、y 与 `properties.orientation` 的 x/y/z/w，取自位姿的底盘部分；
- 只原地替换发生变化的数字，键顺序、缩进、未变化的数字写法和其他字段逐字节保留。

结果接在保存提示后面（已同步 / 已一致 / 未同步的原因）。同步失败不影响已保存的任务图，修正后
再次保存即可重试。GeoJSON 需要对当前登录用户可写，编辑器不会使用 sudo。
...
├── model/         # 流程解析与诊断、图事务和稳定身份、执行器节点定义、地图 GeoJSON 点位合并
├── ssh.rs         # SSH/SFTP：连接、文件操作、远程命令执行
├── ssh/           # agent 认证、原子保存、地图点位同步与取消控制
...
```bash
uv run --no-project python scripts/release_notes.py --tag v0.12.1
...
| 平台 | 文件名 |
|------|--------|
| Linux x64 | `task-graph-editor-v0.12.1+{commit}-linux-x64.tar.gz` |
| macOS ARM64 | `task-graph-editor-v0.12.1+{commit}-macos-arm64.zip`（应用包） |
| Windows x64 | `task-graph-editor-v0.12.1+{commit}-windows-x64.zip` |
